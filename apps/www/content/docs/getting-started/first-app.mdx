...
```typescript title="src/task.interface.ts"
...
```typescript title="src/task.service.ts"
...
```typescript title="src/task.controller.ts"
...
```bash
curl http://localhost:9929/tasks
```
...
```bash
curl http://localhost:9929/tasks/1
```

### Create a new task

```bash
curl -X POST http://localhost:9929/tasks \
...
```bash
curl -X DELETE http://localhost:9929/tasks/1
```
...
<Cards>
  <Card title="Learn About Controllers" href="/docs/overview/controllers" />
  <Card
