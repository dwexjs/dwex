...
export class DwexApplication {
  ...
  private async handleRequest(req: Request): Promise<Response> {
    try {
      ...
      // Find matching route
      const route = this.router.findRoute(request.method, request.url.split('?')[0]);

      if (!route) {
        throw new NotFoundException(`Cannot ${request.method} ${request.url.split('?')[0]}`);
      }
  ...
  private async scanModule(moduleClass: Type<any>): Promise<void> {
    ...
    if (!metadata) {
      throw new Error(
        `${moduleClass.name} is not decorated with @Module()`,
      );
    }
