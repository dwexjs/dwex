...
import { ParamType, type RouteParamMetadata } from '../decorators/params.decorator.js';
...
export class RequestHandler {
  ...
  async handle(route: Route, req: any, res: any): Promise<any> {
    ...
    // Execute guards
    const canActivate = await this.executeGuards(route.guards, executionContext);
    if (!canActivate) {
  ...
  private extractParam(
    ...
    switch (metadata.type) {
      ...
      case ParamType.HEADERS:
        return metadata.data ? req.headers[metadata.data.toLowerCase()] : req.headers;
